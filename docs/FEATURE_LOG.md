...
**Branch / commit liên quan:** `feature/notifications`
...
Sau khi hệ thống có nhiều tính năng (streak, leaderboard, báo cáo câu hỏi, đề thi mới), người dùng không được thông báo về các sự kiện quan trọng ảnh hưởng đến họ. Không có cơ chế nào để biết:

- Mình đã đạt streak bao nhiêu ngày liên tiếp
- Mình vừa lên/xuống hạng sau khi thi thử
- Admin đã xử lý báo cáo câu hỏi của mình
- Có đề thi mới cho môn mình đang học

#### Giải pháp được chọn

Hệ thống thông báo in-app với:
1. **Bell icon 🔔** trong header ProfilePage, kèm badge số đỏ
2. **NotificationPanel** (drawer slide-in từ trên xuống) — danh sách thông báo, đánh dấu đọc
3. **Toast popup** — hiện 7 giây khi có thông báo mới (polling mỗi 30 giây, cộng poll ngay khi tab được focus lại)
4. **5 loại thông báo**: STREAK_MILESTONE, RANK_UP, RANK_DOWN, REPORT_RESOLVED, NEW_EXAM_PAPER (riêng RANK_UP có giao diện nổi bật vàng gold + hiệu ứng phát sáng, khác 4 loại còn lại)

> ⚠️ Cập nhật sau khi S5 test thủ công (xem "Ghi chú kỹ thuật" cuối mục và LESSONS_LEARNED "Vòng 13"): thời lượng toast ban đầu là 4 giây, đã tăng lên 7 giây theo yêu cầu người dùng; đồng thời sửa 1 bug race condition khi 2 toast đến gần nhau.

---

### Phương án kỹ thuật được lựa chọn và lý do

#### 1. Fire-and-forget pattern cho tất cả triggers

**Phương án đã chọn:** Gọi `void notificationService.createNotification(...)` sau khi DB transaction chính commit thành công — không `await`, không block response.

**Phương án bị loại:** Tạo notification TRONG transaction chính (atomic).

**Lý do:**
- Notification không phải dữ liệu nghiệp vụ cốt lõi — mất 1 notification không ảnh hưởng điểm/hạng
- Nếu notification fail không nên rollback toàn bộ giao dịch (VD: nộp bài thi thử)
- Giữ transaction ngắn gọn → ít deadlock, timeout thấp hơn

#### 2. Polling 30 giây thay vì WebSocket

**Phương án đã chọn:** Frontend gọi `GET /api/notifications/unread-count` mỗi 30 giây.

**Phương án bị loại:** WebSocket hoặc Server-Sent Events.

**Lý do:**
- App hiện tại chưa có WebSocket infrastructure; thêm vào tăng độ phức tạp đáng kể
- 30 giây là chu kỳ phù hợp cho sự kiện học tập (không cần real-time ms)
- `unread-count` là endpoint rất nhẹ (1 COUNT query, có index)
- Có thể nâng lên WebSocket sau mà không phá backward compatibility

#### 3. Tách `computeStreaks` sang shared utility

**Lý do:** `progress.service.ts` đã import `practiceService` — nếu `practice.service.ts` lại import `progress.service.ts` sẽ tạo circular dependency. Giải pháp: tách hàm dùng chung vào `src/utils/streak.utils.ts`.

#### 4. Dedup streak notification bằng "first session today"

**Cơ chế:** Kiểm tra số phiên hoàn thành trong ngày hôm nay. Nếu `count !== 1` (không phải phiên đầu tiên) → bỏ qua. Đảm bảo mỗi ngày chỉ gửi 1 notification streak dù học nhiều phiên.

#### 5. Batch insert cho NEW_EXAM_PAPER

**Lý do:** Khi admin bật đề thi mới, có thể có hàng trăm user cần notify. Dùng `prisma.notification.createMany` thay vì N lần `INSERT` riêng lẻ → O(1) round-trip thay vì O(N).
...
| `userId` | `TEXT` FK → `users.id` | Người nhận thông báo |
| `type` | `NotificationType` enum | Loại thông báo |
| `title` | `TEXT` | Tiêu đề ngắn (hiện trong bell panel và toast) |
| `body` | `TEXT` | Nội dung đầy đủ |
| `isRead` | `BOOLEAN` default `false` | Đã đọc hay chưa |
| `targetScreen` | `TEXT` nullable | Màn hình đích khi tap: `progress`/`leaderboard`/`exam`/`null` |
| `metadata` | `JSONB` nullable | Dữ liệu chi tiết theo từng loại |
| `createdAt` | `TIMESTAMP` | Thời gian tạo |

**Index:** `(userId, isRead, createdAt)` — tối ưu cho query "thông báo chưa đọc của user, mới nhất trước"

#### Enum `NotificationType`

| Giá trị | Khi nào trigger |
|---------|----------------|
| `STREAK_MILESTONE` | Hoàn thành phiên ôn tập, streak chạm mốc [7,14,30,60,100] ngày |
| `RANK_UP` | Sau khi nộp bài thi — thứ hạng leaderboard tăng |
| `RANK_DOWN` | Sau khi nộp bài thi — thứ hạng leaderboard giảm |
| `REPORT_RESOLVED` | Admin thay đổi trạng thái báo cáo từ PENDING → REVIEWED/FIXED/DISMISSED |
| `NEW_EXAM_PAPER` | Admin bật `isActive=true` cho đề thi lần đầu |
...
| `RANK_UP` / `RANK_DOWN` | `{ rankBefore: number, rankAfter: number, subject: string }` |
| `REPORT_RESOLVED` | `{ reportId: string, status: string, questionPreview: string }` |
| `NEW_EXAM_PAPER` | `{ subject: string, examPaperTitle: string }` |
...
| Method | Path | Auth | Mô tả |
|--------|------|------|-------|
| `GET` | `/api/notifications` | Bearer token | Danh sách thông báo (phân trang) |
| `GET` | `/api/notifications/unread-count` | Bearer token | Đếm thông báo chưa đọc |
| `PATCH` | `/api/notifications/:id/read` | Bearer token | Đánh dấu 1 thông báo đã đọc |
| `PATCH` | `/api/notifications/read-all` | Bearer token | Đánh dấu tất cả đã đọc |
  ...
  "notifications": [
    {
      ...
      "title": "🔥 Streak 7 ngày!",
      "body": "Tuyệt vời! Bạn đã học liên tiếp 7 ngày.",
      "isRead": false,
...
**Error codes:** `401 MISSING_AUTH_TOKEN`, `401 INVALID_SESSION_TOKEN`
...
**Error codes:** `404 NOTIFICATION_NOT_FOUND`, `403 NOTIFICATION_NOT_OWNED`

#### PATCH /api/notifications/read-all

**Response 200:** `{ "updatedCount": 5 }`

> ⚠️ **Lưu ý route ordering:** `/unread-count` và `/read-all` phải đăng ký TRƯỚC `/:id/read` trong Express để tránh `"unread-count"` và `"read-all"` bị parse như `:id`.

---

### Luồng chạy (Flow)

#### Luồng tổng quan — từ trigger đến người dùng thấy

```
[Sự kiện xảy ra]
       │
       ▼
[Service chính hoàn thành] ──────────────────────────────────────────────────
       │                                                                     │
       │ void (fire-and-forget)                               main response trả về ngay
       ▼
...
INSERT INTO notifications (userId, type, title, body, ...)
       │
       ▼ (30 giây sau)
[Frontend polling GET /api/notifications/unread-count]
       │
       ├─ count > prevCount? ──YES──► GET /api/notifications (page 1, limit 1)
       │                                      │
       │                                      ▼
       │                              [NotificationToast — 7 giây]
       │
       └─ bell badge cập nhật số
              │
              ▼ (user click bell)
       [NotificationPanel mở ra]
              │
              ▼
       [PATCH /api/notifications/:id/read hoặc /read-all]
```

#### Luồng Streak Milestone

```
practice.completeSession() thành công
       │
       ▼
void checkAndFireStreakMilestone(userId)
       │
       ▼
COUNT sessions hôm nay (completedAt trong UTC today)
       │
       ├─ count !== 1 → STOP (đã notify hôm nay rồi)
       │
       └─ count === 1
              │
              ▼
       findMany all completed sessions → computeStreaks(dates)
              │
              ├─ currentStreak NOT IN [7,14,30,60,100] → STOP
              │
              └─ currentStreak IN milestones
                     │
                     ▼
              createNotification(STREAK_MILESTONE, streakDays=N)
```

#### Luồng Rank Change

```
exam.submitExam() bắt đầu
       │
       ▼
getUserCurrentRank(userId, subjectId) → rankBefore
       │
       ▼
[Transaction: chấm điểm, cộng điểm, đóng phiên]
       │
       ▼
void checkAndFireRankChangeNotification(userId, subjectId, rankBefore)
       │
       ▼
getUserCurrentRank(userId, subjectId) → rankAfter
       │
       ├─ rankAfter === null → STOP
       ├─ rankBefore === null → STOP (chưa có hạng trước)
       ├─ rankBefore === rankAfter → STOP
       │
       └─ hạng thay đổi
              ├─ rankAfter < rankBefore → RANK_UP
              └─ rankAfter > rankBefore → RANK_DOWN
```

#### Luồng New Exam Paper (batch)

```
exam.updateExamPaper(isActive=true) — từ false→true
       │
       ▼
void fireNewExamPaperNotifications(subject, title)
       │
       ▼
[2 query song song]
  practiceSession.findMany({ subjectId, completedAt≠null }, distinct userId)
  examSession.findMany({ subjectId }, distinct userId)
       │
       ▼
Union → Set<userId> (deduplicate)
       │
       ▼
notification.createMany([...]) — 1 bulk INSERT
```
...
| File | Vai trò |
|------|---------|
| `backend/prisma/schema.prisma` | +model Notification, +enum NotificationType |
| `backend/prisma/migrations/20260709100000_add_notifications/migration.sql` | Tạo bảng + enum + index |
| `backend/src/services/notification/notification.types.ts` | Types: NotificationItem, CreateNotificationParams, TargetScreen, metadata types |
| `backend/src/services/notification/notification.errors.ts` | NotificationNotFoundError, NotificationNotOwnedError |
| `backend/src/services/notification/notification.service.ts` | CRUD service: create, list, count, markRead, markAllRead |
| `backend/src/services/notification/__tests__/notification.service.test.ts` | 11 unit tests |
| `backend/src/routes/notification.route.ts` | 4 routes với Express Router |
| `backend/src/app.ts` | +import notificationRouter, +2 error codes |
| `backend/src/utils/streak.utils.ts` | computeStreaks() (tách từ progress.service để tránh circular dep) + STREAK_MILESTONES |
| `backend/src/services/practice/practice.service.ts` | +checkAndFireStreakMilestone, +fireReportResolvedNotification, +updateReport (pre-query status) |
| `backend/src/services/exam/exam.service.ts` | +rankBefore query, +checkAndFireRankChangeNotification, +fireNewExamPaperNotifications, updateExamPaper (trigger new paper) |
| `backend/src/services/leaderboard/leaderboard.service.ts` | +getUserCurrentRank() (slim rank query) |
| `backend/src/services/progress/progress.service.ts` | Refactor: import computeStreaks từ utils thay vì define lại |
| `frontend/src/lib/api.ts` | +4 API functions + types (NotificationItem, etc.) |
| `frontend/src/App.tsx` | +polling useEffect, +NotificationToast, +NotificationPanel, +bell icon trong ProfilePage |
| `frontend/src/App.css` | +CSS: header-actions, bell-badge, notif-toast, notif-panel, notif-item |
| `docs/api/drafts/notifications.yaml` | API contract (tạo bởi S1, verify bởi S3) |

---

### Ghi chú kỹ thuật

**Prisma JsonNull vs null:**
Trường `metadata` là `Json?` (nullable). Khi truyền `null` vào Prisma `create`, cần dùng `Prisma.JsonNull` thay vì `null` thuần TypeScript — vì Prisma phân biệt "field có giá trị null" với "field không truyền" (undefined). Metadata không phải JSON object được cast qua `unknown as Prisma.InputJsonValue`.

**Cập nhật sau vòng test thủ công của S5 (2026-07-10) — 2 bug + 2 cải thiện UX:**

1. **Bug — Toast bị tắt sớm khi 2 thông báo đến gần nhau**: `setTimeout` hẹn tắt toast không so sánh `id`, nên timeout của thông báo CŨ có thể tắt nhầm thông báo MỚI trước hạn 7 giây. Sửa bằng cách so sánh `current?.id === newest.id` trong callback trước khi `setToast(null)` (xem Glossary "Toast ID Guard"). Giới hạn: cách sửa chỉ đúng vì tại 1 thời điểm chỉ có 1 toast hiển thị.
2. **Cải thiện — Thời lượng toast**: tăng từ 4 giây lên 7 giây theo yêu cầu người dùng, dễ bắt kịp hơn khi 2 thông báo đến gần nhau.
3. **Cải thiện — Poll ngay khi tab focus lại**: thêm listener `visibilitychange`, gọi `poll()` ngay khi `document.visibilityState === 'visible'` — giảm độ trễ cảm nhận khi trình duyệt throttle `setInterval` ở tab nền (xem Glossary "Poll-on-focus").
4. **Cải thiện — Nổi bật riêng RANK_UP**: thêm CSS nền vàng gold + `@keyframes notif-glow-pulse` cho toast/item loại RANK_UP (`frontend/src/App.css`), phân biệt với 4 loại còn lại (mỗi loại chỉ có viền màu accent bên trái).

Xem chi tiết bài học tại `docs/LESSONS_LEARNED.md` — mục "Bài học bổ sung từ S5 testing (Feature 013)".

**Circular dependency giải pháp:**
```
progress.service → practiceService (đã có)
practiceService → notificationService ✅ (mới thêm, không vòng)
practiceService → streak.utils ✅ (mới thêm, không vòng)
progress.service → streak.utils ✅ (mới thêm, thay thế local function)
```

**Router ordering trong Express:**
```typescript
// ĐÚNG — static path trước
router.get('/unread-count', handler)   // ← phải đăng ký TRƯỚC
router.patch('/read-all', handler)     // ← phải đăng ký TRƯỚC
router.get('/', handler)
router.patch('/:id/read', handler)     // ← parametric CUỐI

// SAI — "unread-count" bị hiểu là :id = "unread-count"
router.patch('/:id/read', handler)
router.patch('/read-all', handler)     // ← không bao giờ match được
```

**prevUnreadRef = -1 (sentinel):**
Khi app mở lần đầu, `prevUnreadRef.current = -1` (chưa poll). Poll đầu tiên set count nhưng KHÔNG hiện toast. Các poll tiếp theo: nếu count tăng → hiện toast. Tránh spam toast với thông báo đã tồn tại từ trước.

**Bugs phát hiện và fix trong S3 review:**
| # | Bug | Fix |
|---|-----|-----|
